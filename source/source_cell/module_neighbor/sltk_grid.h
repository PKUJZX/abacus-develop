...
class Grid
{
  public:
    ...
    void enableCandidateCheckCounting(const bool enabled)
    {
        count_candidate_checks = enabled;
        candidate_check_count = 0;
    }
    std::size_t getCandidateCheckCount() const
    {
        return candidate_check_count;
    }
  private:
    int test_grid;
     ...
     */
    std::size_t Construct_Adjacent_near_box(const FAtom& fatom,
    ...
    int glayerZ=0;
    int glayerZ_minus=0;
    bool count_candidate_checks=false;
    std::size_t candidate_check_count=0;
};
