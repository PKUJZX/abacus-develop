...
constexpr std::ptrdiff_t min_openmp_center_count = 256;
...
void Grid::Construct_Adjacent(const UnitCell& ucell)
{
    ...
    if (count_candidate_checks)
    {
        candidate_check_count = 0;
    }

    std::vector<std::pair<int, int>> center_atoms;
    center_atoms.reserve(static_cast<std::size_t>(ucell.nat));
    for (int i_type = 0; i_type < ucell.ntype; i_type++)
    {
        for (int j_atom = 0; j_atom < ucell.atoms[i_type].na; j_atom++)
        {
            center_atoms.emplace_back(i_type, j_atom);
        }
    }

    std::size_t total_candidate_checks = 0;
    const std::ptrdiff_t center_count = static_cast<std::ptrdiff_t>(center_atoms.size());
#ifdef _OPENMP
#pragma omp parallel for schedule(static) if(center_count > min_openmp_center_count) reduction(+: total_candidate_checks)
#endif
    for (std::ptrdiff_t center_index = 0; center_index < center_count; ++center_index)
    {
        const int i_type = center_atoms[center_index].first;
        const int j_atom = center_atoms[center_index].second;
        ...
        total_candidate_checks += this->Construct_Adjacent_near_box(atom, adjacent_atoms, count_candidate_checks);
    }

    if (count_candidate_checks)
    {
        candidate_check_count = total_candidate_checks;
    }
    ModuleBase::timer::end("Grid", "constru_adj");
}

std::size_t Grid::Construct_Adjacent_near_box(const FAtom& fatom,
                                              std::vector<FAtom*>& adjacent_atoms,
                                              const bool count_candidates)
{
